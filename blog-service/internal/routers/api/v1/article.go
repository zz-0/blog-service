...
//注解说明
//Summary 摘要
//Produce API可产生的MME类型列表--响应类型:JOSN,XML,HTML
//Param 参数格式 从左至右:参数名，入参类型，数据类型，是否必填和注释
//Success 响应成功 从左至右:状态码，参数类型，数据类型和注释
//Failure 响应失败 从左至右:状态码，参数类型，数据类型和注释
//Router 路由:从左至右:路由地址和HTTP方法

//接口文档地址:http://127.0.0.1:8000/swagger/index.html
...
func (a Article) Get(c *gin.Context) {
	//convert.StrTo(c.Param("id")).MustUInt32()---处理路由器获取的上下文，将其中的id类型转换为与文章请求结构体中匹配的类型
	param := service.ArticleRequest{ID: convert.StrTo(c.Param("id")).MustUInt32()}
	response := app.NewResponse(c) //创建上下文示例
...
func (a Article) Create(c *gin.Context) {
	...
	if err != nil {
		...
		response.ToErrorResponse(errcode.ErrorCreateArticlesFail)
		return
	}
...
func (a Article) Update(c *gin.Context) {
	...
	if err != nil {
		...
		response.ToErrorResponse(errcode.ErrorUpdateArticlesFail)
		return
	}
...
func (a Article) Delete(c *gin.Context) {
	...
	if err != nil {
		...
		response.ToErrorResponse(errcode.ErrorDeleteArticlesFail)
		return
	}
